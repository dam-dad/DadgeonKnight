...
//		gc.fillRect(screenX + (Game.tileSize/2)/2, screenY + (Game.tileSize/2), (Game.tileSize/2), (Game.tileSize/2));
		...
		if (timeSinceLastHit < TIME_SHOWING_ATTACK) {
			...
			gc.fillOval(screenX - ((Game.tileSize / 2) * 0.5), screenY - Game.tileSize / 3, Game.tileSize * 1.5,
					Game.tileSize * 1.5);
		}
	}
	...
	public Shape getCollisionShape() {
		return new Rectangle(getPosX() + ((Game.tileSize / 2) * 0.5), getPosY() + (Game.tileSize / 2),
				Game.tileSize / 2, Game.tileSize / 2);
	}

	public Shape getNextCollisionShape(Vector2D v) {
		return new Rectangle((getPosX() + v.getX()) + ((Game.tileSize / 2) * 0.5),
				(getPosY() + v.getY()) + (Game.tileSize / 2), Game.tileSize / 2, Game.tileSize / 2);
	}
